...
function Login({ onEnter }: { onEnter: (r: Role) => void }) {
  const roles = [
    ...
    { id: "candidate" as Role, icon: "⌁", title: "面試者 Demo", text: "驗證身分、開始作答與使用 Mock AI 協作", who: "陳怡安・Junior Data Analyst" },
  ];
...
function HrSummary({ result, status, setModal }: { result: string; status: Status; setModal: (m:"result")=>void }) {
  return <div className="page review-page"><PageTitle title="陳怡安的面試摘要" text="Junior Data Analyst · 技術審核摘要" action={<Badge status={status} />} />
    ...
    <div className="three-summary">{[["作答完成度","3 / 3","全數作答"],["實際作答時間","82 分鐘","預估 75 分鐘"],["AI 對話紀錄","4 則","Mock 協作紀錄"]].map(x=><div className="panel" key={x[0]}><small>{x[0]}</small><strong>{x[1]}</strong><span>{x[2]}</span></div>)}</div>
