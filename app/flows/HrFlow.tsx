...
export function NewCandidate({ candidates, onClose, onSave }: { candidates: Candidate[]; onClose: () => void; onSave: (candidate: Candidate) => void }) {
  ...
  const submit = (event: React.FormEvent) => {
    ...
    if (!name.trim() || !email.trim() || !job || !lead || !due) nextErrors.push("請完整填寫所有必填欄位。");
    if (email.trim() && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email.trim())) nextErrors.push("請輸入有效的 Email 格式。");
    if (due && new Date(due).getTime() <= Date.now()) nextErrors.push("面試期限必須晚於目前時間。");
    setErrors(nextErrors);
    if (nextErrors.length) return;
    setSaving(true);
    onSave({ candidate: name.trim(), email: email.trim().toLowerCase(), job, lead, due: due.replace("T", " "), code: String(Math.floor(100000 + Math.random() * 900000)), url: `talentscope.demo/i/${Date.now()}`, status: "草稿" });
  };
